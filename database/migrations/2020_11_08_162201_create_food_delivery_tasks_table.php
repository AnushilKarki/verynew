...
class CreateFoodDeliveryTasksTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('food_delivery_tasks', function (Blueprint $table) {
            $table->id();
            ...
            $table->foreignId('delivery_id')->references('id')->on('delivery_foods')->onDelete('cascade')->nullable();
            $table->foreignId('delivery')->references('id')->on('deliveries')->onDelete('cascade')->nullable();
            $table->float('head_earning')->nullable();
    ...
    {
        Schema::dropIfExists('food_delivery_tasks');
    }
}
